...
CREATE USER IF NOT EXISTS 'paulo' IDENTIFIED BY '123'; -- criando usuário (mesma coisa se usasse 'paula'@'%') -- se paulo existir, para evitar dar erro -- identified by: criando senha -- evite senhas simples
...
CREATE USER IF NOT EXISTS 'paulo2'@'%' -- por padrão cria assim, pode acessar este BD de qualquer lugar e em qualquer máquina (mesma coisa se não tiver @'%')  (PODERIA ADICIONAR UM IP ESPECÍFICO DE UMA MÁQUINA POR QUESITOS DE SEGURANÇA)
IDENTIFIED BY '123'; 
...
ON cliente2.* -- * representa todos os objetos do BD cliente2. Se quisesse todos os bancos seria *.*
TO paulo; -- para determinado user

GRANT SELECT, INSERT, UPDATE, DELETE, EXECUTE -- Aqui iremos dar alguns privilegio, para paulo, acessando de qq local, a todas as tabelas do banco cliente2
ON cliente2.* 
TO paulo2;

-- Usa-se flush privilegies quando se altera a tabela de controle (mas não é recomendado). Se não desse o flush, o user não teria acesso. Ou parar o serviço do sql e iniciar novamente.
...
CREATE USER IF NOT EXISTS 'MARIA'@'localhost' -- maria pode acessar do localhost (servidor do mysql local), poderia ser de outro servidor, VirtualBox, por exemplo
IDENTIFIED BY '123'; 

GRANT SELECT
ON cliente2.order -- pode acessar por SELECT apenas a tabela order do bd cliente2
TO MARIA@localhost;
...
-- DBA precisa criar users, mas ficar dando grant e revoke nos previlegies é melhor usar role (papel) para facilitar o processo, por exemplo, DBA senior, junior etc. para não ter que ficar dando para cada usuário particularmente.
...
GRANT ALL ON CLIENTE2.* TO 'app_desenvolvedor'; -- dando todos os privilégios do BD cliente2 para todos os objetos para a role app_desenvolvedor
GRANT SELECT ON CLIENTE2.* TO 'app_leitura'; -- dando priviçégio de leitura  do BD cliente2 para todos os objetos para a role app_leitura
GRANT INSERT, UPDATE, DELETE ON CLIENTE2.* TO 'app_gravacao'; -- dando privilégio de inserção, atualizar e deletar do BD cliente2 para todos os objetos para a role app_gravacao
...
CREATE USER 'dev1'@'localhost' IDENTIFIED BY 'dev1pass'; -- pode acessar apenas da máquina local
...
-- FLUSH PRIVILEGES; -- atualiza privilegios 
...
GRANT 'app_leitura' TO 'read_user1'@'localhost', 'read_user2'@'localhost'; -- separado por vírgula os dois users
GRANT 'app_leitura', 'app_gravacao' TO 'rw_user1'@'localhost'; -- separado por vírgula os dois roles

SHOW GRANTS FOR 'app_leitura';  -- verificando privilegios de uma role

SHOW GRANTS FOR 'app_desenvolvedor'; -- previlegios da role
SHOW GRANTS FOR 'dev1'@'localhost'; -- privilegios do user
SHOW GRANTS FOR 'dev1'@'localhost' USING 'app_desenvolvedor'; -- verificando todos os privilegios dados ao dev1 no papel desenvolvedor, pois poderia estar usando outros papéis
