...
select *
from cliente2.customer C JOIN cliente2.order O-- JOIN e INNER JOIN é a mesma coisa (Traz dados que existam nas duas tabelas.)
ON C.id=O.customerid;

SELECT *
FROM product AS 
p JOIN orderitem AS oi ON p.id = oi.productid 
JOIN `order` AS o ON o.id = orderid;
...
SELECT  O.OrderNumber, OrderDate AS Datetime, 
  ...
  INNER JOIN OrderItem I ON O.Id = I.OrderId -- não importa a ordem dos joins, desde que as tabelas estejam vinculados
...
UPDATE Supplier
   SET City = 'Oslo', -- altera determinado dado de certa coluna
       Phone = '(0)1-953530', 
       Fax = '(0)1-953555'
 WHERE Id = 2; -- se esquecer de usar a cláusula WHERE, altera de todas as linhas -- CUIDADO. Fazer backup antes. importante especificar WHERE, caso contrário todos os dados serim atualizados
-- COMMIT; (atualização) já é feito automaticamente (pode mudar o padrão, mas não é indicado)
 
select * from Supplier where Id = 2;

-- TRANSACOES
-- Transação garante integridade: manda para log, se cair servidor, volta para log
...
START TRANSACTION; -- como DBA, ideal usar START TRANSACTION/BEGIN (todos os comandos dentro da transação serão alterados apenas para sua sessão, as outras sessões dos outros usuários não poderão ver, apenas depois do COMMIT, que é feiro automaticamente.
UPDATE Product -- rodando sem START TRANSACTION não é possível desfazer o UPDATE (ROLLBACK)
   SET IsDiscontinued = 0
      where IsDiscontinued=1; -- quando não é chave primária dá erro no where, podendo-se usar SET
SET SQL_SAFE_UPDATES = 0;
select * from Product;
ROLLBACK; -- desfaz antes de gravar definitivamente quando se usa START TRANSACTION e não termina com COMMIT, ou apenas dá o UPDATE

-- SET SQL_SAFE_UPDATES = 0 -- dessativa atualização segura apenas por meio de chaves primárias -- SET SQL_SAFE_UPDATES = 1 ativa
...
START TRANSACTION; -- sem COMMIT
UPDATE Product
   SET IsDiscontinued = 0 
      where IsDiscontinued=1 and id<>0; -- forçando sem SET SQL_SAFE_UPDATES passando id como 0, por exemplo
select * from Product;
...
SET SQL_SAFE_UPDATES = 0; -- burlar o uso do id desativando update seguro (ou desligar em edit -> preferences -> safe)
START TRANSACTION;
UPDATE Product
   SET IsDiscontinued = 0
      where IsDiscontinued=1;
select * from Product;
SET SQL_SAFE_UPDATES = 1; -- reativa update seguro

-- Rollback tran Funnciona apenas no SQL SERBER
-- Para MYSQL apenas ROLLBACK ou COMMIT
ROLLBACK; -- desfaz o START TRANSACTION	
select * from Product;
...
BEGIN ; -- mesma coisa que o START TRANSACTION
UPDATE Product 
...
ROLLBACK; -- não funciona mais após COMMIT (MySQL não dá erro, no SQL SERVER sim) -- funciona apenas com comandos DML (INSERT, UPDATE E DELETE)
-------------------------------
------ DELETES
-------------------------------

select * from orderitem;
BEGIN; -- área transitória até usar commit (ideal para garantir que os dados foram alterados corretamente, mas tem que se mudar rápido logo, pois estarão em lock, ou seja não poderão ser usados.)
DELETE FROM orderitem ; -- apaga todos os dados, necessário usar cláusula WHERE para filtrar (TRUNCATE deleta mais rápido e minimiza os dados no log)
select * from orderitem;

ROLLBACK; -- se desse commit não poderia ser desfeito mais, apenas c/ backup do bd
select * from orderitem;
...
-------------------------------
 ...
 SELECT * FROM orderitem; -- criando tabela para teste com os dados de outra para usar truncate (tabela inexistente, inserindo dados) 
 SELECT * FROM neworderitem;

TRUNCATE TABLE neworderitem; -- apaga todos os dados da tabela, muito mais rápido que usar DELETE FROM neworderitem (É AUTOCOMMIT, por isso nç´o faz sentido usar BEGIN/START TRANSACTION e muito menos ROLLBACK, não é pos´sível retornar como no DELETE
SELECT * FROM neworderitem;
...
INSERT INTO Customer (id, FirstName, LastName, City, Country, Phone) -- inserindo valores na tabela
...
INSERT INTO Customer (FirstName, LastName, City, Country, Phone) -- id não está default, não vai funcionar
SELECT LEFT(ContactName, 5), -- pegando os 5 primeiros caracteres usando LEFT e jogando dentro da primeira coluna Firstname      -- é possível inserir dados a partir da mesma tabela e ou de outras tabelas -- normal vindo de outras tabelas
       SUBSTRING(ContactName, 2, 2),  -- posicao 2, quantidade 2 de caracteres de contactname  e jogando para lastname
       City, Country, Phone
  FROM Supplier -- outra tabela
 WHERE CompanyName = 'Bigfoot Breweries'; -- filtrando dados
 ...
 ALTER TABLE Customer MODIFY id INTEGER NOT NULL AUTO_INCREMENT; -- não posso alterar a primary key pois é foreign key em outras tabelas (restrict), precisa alterar
...
SET FOREIGN_KEY_CHECKS = 0; -- desligando relacionamento (foreign key) temporariamente (feito com muito cuidado), não tendo necessidade de apagá-la, mudar a priary key da sua tabela, depois alterar a foreign key... (muito trabalhoso)
ALTER TABLE `cliente2`.`customer` CHANGE COLUMN `id` `id` INT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT = 200; -- altera depois de desligado o relacionamento -- começa a contar auto_increment a partir do valor 200
-- ALTER TABLE customer MODIFY COLUMN id INT NOT NULL AUTO_INCREMENT, AUTO_INCREMENT = 200; -- praticamente a mesma coisa do de cima 
SET FOREIGN_KEY_CHECKS = 1; -- depois de alterado mudar para ligado imediatamente
-- agora ja poder rodar o mesmo comando que o id sera gerado a partir do valor 102

INSERT INTO Customer (FirstName, LastName, City, Country, Phone) -- auto_increment implícito
SELECT LEFT(ContactName, 5), 
 ...
 DELETE FROM customer WHERE id = 201;
 
select * from Customer;

-- Sempre usar begin/start transaction quando usar INSERT, UPDATE E DELETE por questão de segurança
